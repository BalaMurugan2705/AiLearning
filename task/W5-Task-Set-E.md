...
Your docs assistant has been running all week and your trace file has grown past a thousand lines that nobody has read. The DX lead says it 'sometimes gives outdated code', which is not a bug report. Draw a genuinely random sample, read every trace by hand without fixing anything, and turn what you saw into a ranked list your manager can act on.
...
4. Cluster your sentences into 4-7 named failure modes. Names must be legible to a stranger: 'answers with the v2 method signature for a v3 question' beats 'retrieval issue'. Report frequency as a count AND a percent of the 20, plus a severity (ships broken code to a user's repo / merely annoys the reader), plus one example trace_id per mode.
5. Write a falsifiable, dated prediction with numbers: name the ONE mode you will attack next week, the specific change, and the exact delta you expect, e.g. 'filtering on sdk_version drops the wrong-version mode from 30% to under 10%'. Commit it to git with today's date.
...
- **Writing 'retrieval issue' or 'hallucination' as a mode name. Neither tells your DX lead what to do, and both are diagnoses smuggled in as observations — the diagnosis is next week's job.**
