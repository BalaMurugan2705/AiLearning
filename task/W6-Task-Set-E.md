...
Your eval prints a helpfulness score for every answer your docs assistant gives about the API, and that whole number comes out of an LLM judge nobody has ever checked against a human. DevRel is about to ship the assistant on the strength of that score, while v2 endpoints keep getting recommended to v3 users. Prove the judge agrees with you — or find out it doesn't — and move the agreement figure with evidence.
...
1. Bring the eval set to 25+ cases, each tagged with one Week-5 taxonomy mode, and add at least 2 regression cases replayed verbatim from real failed docs traces; one command still runs everything and prints pass rate by mode.
2. Move at least 2 criteria out of the judge and into deterministic assertions (the code sample parses, every endpoint path mentioned exists in the OpenAPI spec, the API version is stated, no symbol from the deprecations list appears without a migration note) and delete those criteria from the judge prompt; report the count of assertions vs judged criteria.
...
Add RAGAS faithfulness and context precision to the docs-backed cases. Find one answer that scores 0.9+ faithfulness while grounding itself in the v2 page for a v3 question — confidently, faithfully wrong — and show the two numbers plus why the overall average hides it.
...
- **Paying a model to check whether a code sample parses or whether an endpoint exists in the spec — a parser and a spec lookup do that for free and never have an off day.**
- **Scoring answer helpfulness 1-10 and calling within-1 a match; the model cannot tell a 6 from a 7 and neither can you, and the tolerance inflates agreement into meaninglessness.**
- **Reporting one overall pass rate; the average will happily hide a regression on the version-confusion mode while the conceptual-explanation mode carries the number.**
